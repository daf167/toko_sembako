        ...
        <div class="table-wrap bg-white p-3">
            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    <thead><tr><th>Tanggal</th><th>Barang</th><th>Tipe</th><th>Jumlah</th><th>Stok Awal</th><th>Stok Akhir</th><th>Petugas</th></tr></thead>
                    <tbody>
                        @forelse($logs as $log)
                            @php
                                $itemName = $log->item?->name ?? ($log->item_name_snapshot ? 'Produk dihapus - '.$log->item_name_snapshot : 'Produk dihapus');
                            @endphp
                            <tr><td>{{ $log->date->format('d/m/Y') }}</td><td>{{ $itemName }}</td><td>{{ $log->type }}</td><td>{{ $log->quantity }}</td><td>{{ $log->stock_before ?? '-' }}</td><td>{{ $log->stock_after ?? '-' }}</td><td>{{ $log->user->name }}</td></tr>
                        @empty
